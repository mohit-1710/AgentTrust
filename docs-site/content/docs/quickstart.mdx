...
|---|---|---|---|
| Local install — `npx -y @agenttrust-sdk/mcp@latest` | 21 (10 read + 8 write + 3 discovery) | Your key (layered chain: `PRIVATE_KEY` → `KEY_PATH` → `an EVM keystore`) | Building with AgentTrust. Creating policies, attesting capabilities, emitting feedback. Your key stays on your machine; you own every contract account you create. |
