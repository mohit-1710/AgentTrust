...
[`mcp.agenttrust.tech`](https://mcp.agenttrust.tech) is the hosted MCP HTTP endpoint. Always-on, testnet-bound, free to hit, no install required for clients that speak `StreamableHTTPServerTransport`.
